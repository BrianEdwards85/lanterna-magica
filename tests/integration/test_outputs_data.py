...
async def test_get_by_ids_multiple(pool):
    ...
    assert_that(str(b["id"])).described_as("unrequested output absent").is_not_in(
        returned_ids
    )
...
async def test_list_outputs_basic(pool):
    ...
    assert_that(templates).described_as("both outputs listed").contains(
        "/a.json", "/b.json"
    )
...
async def test_list_outputs_excludes_archived_by_default(pool):
    ...
    b = await outputs.create(
        path_template="/archive.json", format="json", dimension_ids=[]
    )
    ...
    assert_that(templates).described_as("archived excluded").does_not_contain(
        "/archive.json"
    )
...
async def test_list_outputs_pagination(pool):
    ...
    for i in range(5):
        await outputs.create(
            path_template=f"/out-{i:02d}.json", format="json", dimension_ids=[]
        )

    page1 = await outputs.list(first=2)
    assert_that(page1["edges"]).described_as("page 1 size").is_length(2)
    assert_that(page1["page_info"]["has_next_page"]).described_as(
        "has next page"
    ).is_true()
...
async def test_archive_output(pool):
    ...
    outputs = Outputs(pool)
    created = await outputs.create(
        path_template="/x.json", format="json", dimension_ids=[]
    )
