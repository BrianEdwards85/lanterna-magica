...
async def test_create_shared_value(client):
    ...
    assert_that(sv["archivedAt"]).described_as("new shared value should not be archived").is_none()
...
async def test_shared_values_by_ids_multiple(client):
    ...
    assert_that(ids).described_as("fetched shared value ids").contains(sv1["id"], sv2["id"])


async def test_shared_values_by_ids_unknown_returns_empty(client):
    body = await gql(client, SHARED_VALUES_BY_IDS, {"ids": ["00000000-0000-0000-0000-ffffffffffff"]})
    assert_that(body["data"]["sharedValuesByIds"]).described_as("non-existent id returns empty list").is_empty()
...
async def test_shared_values_list(client):
    ...
    assert_that(nodes(body["data"]["sharedValues"]["edges"])).described_as("shared values list").extracting(
        "name"
    ).contains("db_password", "api_key")
...
async def test_update_shared_value(client):
    ...
    assert_that(parse_dt(updated["updatedAt"])).described_as("updatedAt advanced").is_after(parse_dt(sv["updatedAt"]))
...
async def test_unarchive_shared_value(client):
    ...
    assert_that(restored["archivedAt"]).described_as("archivedAt cleared after unarchive").is_none()
...
async def test_search_by_name(client):
    ...
    assert_that(results[0]["name"]).described_as("matched shared value name").is_equal_to("db_password")
...
async def test_search_respects_pagination(client):
    ...
    assert_that(page["pageInfo"]["hasNextPage"]).described_as("more search results available").is_true()
