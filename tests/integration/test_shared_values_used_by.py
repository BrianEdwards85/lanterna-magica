...
async def _create_configuration_with_substitution(
    client, dimension_ids, body_json, substitutions
):
    variables = {
        "input": {
...
async def test_used_by_dataloader_multiple_shared_values_batched(client):
    ...
    assert_that(ids_for_sv1).described_as("cfg1 should appear in sv1.usedBy").contains(
        cfg1["id"]
    )
    assert_that(ids_for_sv2).described_as("cfg2 should appear in sv2.usedBy").contains(
        cfg2["id"]
    )
...
async def test_set_noncurrent_revision_to_current(client):
    ...
    body = await gql(
        client, SET_REVISION_CURRENT, {"id": rev1["id"], "isCurrent": True}
    )
...
async def test_deactivate_current_revision(client):
    ...
    body = await gql(
        client, SET_REVISION_CURRENT, {"id": rev["id"], "isCurrent": False}
    )
    result = body["data"]["setRevisionCurrent"]
...
async def test_set_current_only_affects_same_scope(client):
    ...
    rev_staging = await create_revision(
        client, sv["id"], [svc["id"], env2["id"]], "staging"
    )
    ...
    rev_prod2 = await create_revision(
        client, sv["id"], [svc["id"], env1["id"]], "prod2"
    )
